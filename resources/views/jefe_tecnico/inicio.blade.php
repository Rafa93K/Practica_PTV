    ...
    @include('layouts.header')  
                                                    ...
                                                    <form action="{{ route('jefe_tecnico.asignar') }}" method="POST" class="flex items-center gap-2">
                                                        ...
                                                        <div class="flex flex-col gap-1">
                                                            <input type="text" name="hora" id="horaTecnico" required
                                                                class="w-20 text-center px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                                                                value="{{ date('H:i') }}">
                                                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="text-gray-400 text-sm uppercase tracking-wider border-b border-gray-100">
                                        <th class="pb-4 font-semibold">Técnico</th>
                                        <th class="pb-4 font-semibold text-center">Incidencias Resueltas</th>
                                        <th class="pb-4 font-semibold text-right">Media de resolución (minutos)</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-50">
                                    @foreach($mediasTecnicos as $media)
                                        <tr class="group hover:bg-gray-50/50 transition-colors">
                                            <td class="py-4 pr-4">
                                                <div class="font-bold text-gray-700">{{ $media->nombre }} {{ $media->apellido }}</div>
                                            </td>
                                            <td class="py-4 px-4 text-center">
                                                <div class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 font-medium text-sm">
                                                    {{ $media->incidencias_resueltas }}
                                                </div>
                                            </td>
                                            <td class="py-4 pl-4 text-right">

                                                {{-- Si es mayor a 0 y la media de resolucion no es nula --}}
                                                @if($media->incidencias_resueltas > 0 && $media->media_resolucion !== null)
                                                    <div class="font-bold text-gray-800 text-lg">{{ $media->media_resolucion }} <span class="text-sm font-normal text-gray-500">mins</span></div>
                                                @else
                                                    <div class="font-bold text-gray-400 italic">Sin datos</div>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
        ...
        <script>
            ...
            flatpickr("#horaTecnico", {
                ...
                minuteIncrement: 15, //Incremento de minutos
            })
        </script>
    </body>
</html>