...
<html lang="es">
    ...
    <body class="bg-gray-50 flex flex-col min-h-screen">
            ...
            <div class="grid md:grid-cols-2 gap-8">
                ...
                <div class="bg-white p-6 rounded-xl shadow">
                    ...
                    <form action="{{ route('marketing.filtrar') }}" method="GET" class="flex items-center gap-2">
                        <span class="text-sm text-gray-500">Filtrar por periodo:</span>

                        <input type="text" name="periodo" id="periodo"
                            value="{{ $añoSeleccionado && $mesSeleccionado ? $añoSeleccionado . '-' . $mesSeleccionado : '' }}" 
                            placeholder="Seleccione mes..."
                            class="border border-gray-300 rounded-lg px-4 py-1.5 bg-white text-sm focus:ring-indigo-500 focus:border-indigo-500">

                        <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm text-sm font-medium">
                            Filtrar
                        </button>
                        @if($mesSeleccionado || $añoSeleccionado)
                            <a href="{{ route('marketing.inicio') }}" class="text-xs text-gray-400 hover:text-indigo-600">Limpiar</a>
                        @endif
                    </form>
        ...
        <script>
            //Configuración de Flatpickr para solo Mes y Año
            flatpickr("#periodo", {
                locale: "es",
                plugins: [
                    new monthSelectPlugin({
                        shorthand: true, //Muestra meses abreviados
                        dateFormat: "Y-m", //Lo que se envia al servidor
                    })
                ]
            });
