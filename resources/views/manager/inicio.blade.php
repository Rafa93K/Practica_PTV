...
<html lang="es">
    ...
    <body class="bg-gray-50 flex flex-col min-h-screen">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-md">
                <form action="{{ route('manager.inicio') }}" method="GET" class="flex flex-wrap items-end gap-4">
                    <div class="flex-1 min-w-[200px]">
                        ...
                        <input type="text" name="fecha_inicio" id="fecha_inicio" value="{{ $fechaInicio }}" placeholder="Seleccione fecha..."
                    ...
                    <div class="flex-1 min-w-[200px]">
                        ...
                        <input type="text" name="fecha_fin" id="fecha_fin" value="{{ $fechaFin }}" placeholder="Seleccione fecha..."
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 bg-white">
                    </div>
                    <div>
                        <button type="submit"
                            class="bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700 transition font-medium shadow-sm">
                            Filtrar
                        </button>
                        <a href="{{ route('manager.inicio') }}"
                            class="ml-2 text-gray-500 hover:text-indigo-600 text-sm font-medium">Limpiar</a>
                    </div>
                </form>
            </div>
        </section>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-md hover:shadow-lg transition">
                    ...
                    <div class="mt-2">
                        @if($totalIncidencias == 0)
                            <p class="text-3xl font-bold text-gray-400">{{ $totalIncidencias }}</p>
                        @else
        ...
        <!-- Importar localización en español para Flatpickr -->
